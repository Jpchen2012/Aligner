			...
			if(((data->arglist+j)->resbuffer+(data->arglist+j)->ip)->iscomp && (!(data->arglist+j)->isprint)){	
				for(i = (data->arglist+j)->ip ; i < ((data->arglist+j)->ip+(data->arglist+j)->bufsize); i++){
					if(((data->arglist+j)->resbuffer+i)->iscomp && (!((data->arglist+j)->resbufferb+i)->iscomp)){	
							...
							if((((data->arglist+j)->resbuffer+i)->flag/4)%2 == 1){
								if(jcb == -1)
									...
									printf("%d\t%s%lu\t%d\t%s\t", ((data->arglist+j)->resbuffer+i)->flag, chrlist[jca], posa, ((data->arglist+j)->resbuffer+i)->mapscore, ((data->arglist+j)->resbuffer+i)->cigar);
									if(jcb == -1)
										printf("=\t%lu\t0\t", posa);
									else if(jca == jcb)
										printf("=\t%lu\t%d\t", posb, deltaposa);
									else
										printf("%s%lu\t0\t", chrlist[jcb], posb);
													
									printf("%s\t%s\t", ((data->arglist+j)->seqbuffer+i)->seq, ((data->arglist+j)->seqbuffer+i)->qual);
							...
							if(isid == 1){
									...
									printf("\n");
							if(isspace){
								sscanf(((data->arglist+j)->seqbufferb+i)->sid, "%s", psid);
								printf("%s\t", (psid+1));
							}
							else if(!isslash){
								printf("%s\t", (((data->arglist+j)->seqbufferb+i)->sid+1));
							}
							else
								printf("%*.*s\t", 2, (strlen(((data->arglist+j)->seqbufferb+i)->sid)-3), (((data->arglist+j)->seqbufferb+i)->sid+1));
							if((((data->arglist+j)->resbufferb+i)->flag/4)%2 == 1){
								if(jca == -1)
									printf("%d\t*\t0\t0\t*\t*\t0\t0\t%s\t%s", ((data->arglist+j)->resbufferb+i)->flag, ((data->arglist+j)->seqbufferb+i)->seq, ((data->arglist+j)->seqbufferb+i)->qual);
								else
									printf("%d\t%s%lu\t0\t*\t=\t%lu\t0\t%s\t%s", ((data->arglist+j)->resbufferb+i)->flag, chrlist[jca], posa, posa, ((data->arglist+j)->seqbufferb+i)->seq, ((data->arglist+j)->seqbufferb+i)->qual);
							}
							else{
									printf("%d\t%s%lu\t%d\t%s\t", ((data->arglist+j)->resbufferb+i)->flag, chrlist[jcb], posb, ((data->arglist+j)->resbufferb+i)->mapscore, ((data->arglist+j)->resbufferb+i)->cigar);
								if(jca == -1)
									printf("=\t%lu\t0\t", posb);
								else if(jca == jcb)
									printf("=\t%lu\t%d\t", posa, deltaposb);
								else
									printf("%s%lu\t0\t", chrlist[jca], posa);
														
								printf("%s\t%s\t", ((data->arglist+j)->seqbufferb+i)->seq, ((data->arglist+j)->seqbufferb+i)->qual);
														
								printf("NM:i:%d", ((data->arglist+j)->resbufferb+i)->nmis);
							}									
							if(isid == 1){
								printf("\tRG:Z:%s\n", idname);
							}
						...
						((data->arglist+j)->resbufferb+i)->iscomp = true;
					...
					if((data->arglist+j)->nprint == (data->arglist+j)-> bufsize){
						...
						totlmapped += (data->arglist+j)->nlmapped;
						totrmapped += (data->arglist+j)->nrmapped;
						totlunimapped += (data->arglist+j)->nlunimapped;
						totrunimapped += (data->arglist+j)->nrunimapped;
...
int main(int argv, char* argc[])
{
		...
		if(argv <= 1){
			...
			fprintf(stderr, "palinger index reads1.fq reads2.fq -n nthreads -id sample_id -sm sample_name -pl platform_name -lb library_name > results.sam \n");
			exit(0);
		}
		...
		uint32_t* npos = (uint32_t*) calloc(nns, sizeof(uint32_t)); // positions of N segs
		...
		gzFile gzfqb;
		if(isgz){
			gzfq = gzopen(argc[2], "rb");
			gzfqb = gzopen(argc[3], "rb");
			gzbuffer(gzfq, gzbufsize);
			gzbuffer(gzfqb, gzbufsize);
			
			if((!gzfq)||(!gzfqb)) {
				fprintf(stderr, "read fastq file err \n");
				exit(0);
			}
			if((pret = gzgets(gzfq, firstid, 512)) == NULL){
				fprintf(stderr, "read fastq file err \n");
				exit(0);
			}
			if((pret = gzgets(gzfqb, firstidb, 512)) == NULL){
				fprintf(stderr, "read fastqb file err \n");
				exit(0);
			}
			firstid[strlen(firstid) - 1] = '\0';
			gzgets(gzfq, firstseq, 512);
			firstseq[strlen(firstseq) - 1] = '\0';
			gzgets(gzfq, plus, 512);
			plus[strlen(plus) - 1] = '\0';
			gzgets(gzfq, firstqual, 512);
			firstqual[strlen(firstqual) - 1] = '\0';
			
			firstidb[strlen(firstidb) - 1] = '\0';
			gzgets(gzfqb, firstseqb, 512);
			firstseqb[strlen(firstseqb) - 1] = '\0';
			gzgets(gzfqb, plus, 512);
			plus[strlen(plus) - 1] = '\0';
			gzgets(gzfqb, firstqualb, 512);
			firstqualb[strlen(firstqualb) - 1] = '\0';
		} 
		else{
			fq = fopen(argc[2],"r");
			fqb = fopen(argc[3],"r");
			if((fq == NULL)||(fqb == NULL)){
				fprintf(stderr, "file arg is err \n");
				exit(0);
			}
			else{
				...
				if((pret = fgets(firstidb, 512, fqb)) == NULL){
					fprintf(stderr, "read fastqb file err \n");
					exit(0);
				}
				firstid[strlen(firstid) - 1] = '\0';
				...
				firstidb[strlen(firstidb) - 1] = '\0';
				fgets(firstseqb, 512, fqb);
				firstseqb[strlen(firstseqb) - 1] = '\0';
				fgets(plus, 512, fqb);
				plus[strlen(plus) - 1] = '\0';
				fgets(firstqualb, 512, fqb);
				firstqualb[strlen(firstqualb) - 1] = '\0';
			}
		}			
				...
				if(pret != NULL){
					...
					readlenb = strlen(firstseqb);
					sidlen = strlen(firstid);
					sidlenb = strlen(firstidb);
					if((readlen > 17)&&(readlenb > 17)){
						subsize = 50000*100/(readlen+readlenb);
						buffersize = subsize*nthreads;
					}
					else{
						fprintf(stderr, "fastq file has errs, read length is too short!\n");
						exit(0);
					}
					for(i = 0; i < nchr; i++){
						printf("@SQ\tSN:%sLN:%lu\n", chrlist[i],chrlen[i]);
					...
					totnfq = 1;
					iseq = 1;
					for(i = 0; (i < strlen(firstid))&&(ret == 0); i++){
		...
		reads* seqbufferb = (reads*) calloc(buffersize, sizeof(reads));
		res* resbufferb = (res*) calloc(buffersize, sizeof(res));
		...
		for(i = 0; (readlen*readlenb > 0)&&(i < buffersize); i++){
				 (seqbuffer+i)->sid = (char*)calloc((sidlen+50), sizeof(char));
				 (seqbuffer+i)->seq = (char*)calloc(readlen, sizeof(char));
				 (seqbuffer+i)->qual = (char*)calloc(readlen, sizeof(char));				
				 (resbuffer+i)->flag = 77;
				 
				 (seqbufferb+i)->sid = (char*)calloc((sidlenb+50), sizeof(char));
				 (seqbufferb+i)->seq = (char*)calloc(readlenb, sizeof(char));
				 (seqbufferb+i)->qual = (char*)calloc(readlenb, sizeof(char));
				 (resbufferb+i)->flag = 141;
		}
		...
		for(i = 0; i < nthreads; i++){
				...
				(arglist+i)->seqbufferb = seqbufferb;
				(arglist+i)->resbufferb = resbufferb;
				(arglist+i)->ip = i*subsize;
				(arglist+i)->isfw = isfw;
				(arglist+i)->bufsize = subsize;
				(arglist+i)->nprint = 0;
				(arglist+i)->iscomp = false;
				(arglist+i)->isprint = true;
		...
		dataarg.readlen = readlen;
		dataarg.readlenb = readlenb;
		bool ispc = false;
		bool isfinish = false;
		bool goodseq = false;
				...
				strcpy(seqbufferb->sid, firstidb);
				strcpy(seqbufferb->seq, firstseqb);
				strcpy(seqbufferb->qual, firstqualb);
				
				while(!isfinish){
					if(ithread != -1){
						if(isgz){
							if((pret = gzgets(gzfq, (seqbuffer+iseq)->sid, 512)) != NULL){
								(seqbuffer+iseq)->sid[strlen((seqbuffer+iseq)->sid) - 1] = '\0';
								gzgets(gzfq, (seqbuffer+iseq)->seq, 512);
								(seqbuffer+iseq)->seq[strlen((seqbuffer+iseq)->seq) - 1] = '\0';
								gzgets(gzfq, plus, 512);
								plus[strlen(plus) - 1] = '\0';
								gzgets(gzfq, (seqbuffer+iseq)->qual, 512);
								(seqbuffer+iseq)->qual[strlen((seqbuffer+iseq)->qual) - 1] = '\0';
							}
							if((pret = gzgets(gzfqb, (seqbufferb+iseq)->sid, 512)) != NULL){
								(seqbufferb+iseq)->sid[strlen((seqbufferb+iseq)->sid) - 1] = '\0';
								gzgets(gzfqb, (seqbufferb+iseq)->seq, 512);
								(seqbufferb+iseq)->seq[strlen((seqbufferb+iseq)->seq) - 1] = '\0';
								gzgets(gzfqb, plus, 512);
								plus[strlen(plus) - 1] = '\0';
								gzgets(gzfqb, (seqbufferb+iseq)->qual, 512);
								(seqbufferb+iseq)->qual[strlen((seqbufferb+iseq)->qual) - 1] = '\0';
								goodseq = false;
								if((strlen((seqbuffer+iseq)->seq) == readlen)&&(strlen((seqbufferb+iseq)->seq) == readlenb)){
									if((strlen((seqbuffer+iseq)->qual) == readlen)&&(strlen((seqbufferb+iseq)->qual) == readlenb)){
										iseq++;
										totnfq ++;
										goodseq = true;
									}
							...
							if((pret = fgets((seqbufferb+iseq)->sid, 512, fqb)) != NULL){
								(seqbufferb+iseq)->sid[strlen((seqbufferb+iseq)->sid) - 1] = '\0';
								fgets((seqbufferb+iseq)->seq, 512, fqb);
								(seqbufferb+iseq)->seq[strlen((seqbufferb+iseq)->seq) - 1] = '\0';
								fgets(plus, 512, fqb);
								plus[strlen(plus) - 1] = '\0';
								fgets((seqbufferb+iseq)->qual, 512, fqb);
								(seqbufferb+iseq)->qual[strlen((seqbufferb+iseq)->qual) - 1] = '\0';
								//iseq++;
								//totnfq ++;
								goodseq = false;
					...
					if((iseq%subsize == 0)||(pret == NULL)){
						if(ithread != -1){
							...
							if((arglist+ithread)->bufsize > 0){
							for(i = (arglist+ithread)->ip; i < ((arglist+ithread)->ip + (arglist+ithread)-> bufsize); i++){
									(resbuffer+i)->iscomp = false;
									(resbufferb+i)->iscomp = false;
							}
							(arglist+ithread)->isprint = false;
							(arglist+ithread)->iscomp = false;
							(arglist+ithread)->nprint = 0;
							
							ret = pthread_create((tid+ithread), NULL, aln_pe, (arglist+ithread));
								if(ret != 0){
	...
	if(isgz){
		...
		for(i = 0; i < buffersize; i++){
			...
			if(strlen((seqbufferb+i)->seq) > 0){
				free((seqbufferb+i)->sid);
				free((seqbufferb+i)->seq);
				free((seqbufferb+i)->qual);
			}
		}
		free(seqbuffer);
		free(resbuffer);
		...
		free(chrlen);
		free(chrpos);
		fprintf(stderr, "number of total pairs\t%d\n", totnfq);
		fprintf(stderr, "number of properly mapped pairs\t%d\t%.2f\%\n", totmapped, 100*totmapped/(totnfq+0.00001));
		fprintf(stderr, "number of uniquely mapped pairs\t%d\t%.2f\%\n", totunimapped, 100*totunimapped/(totnfq+0.00001));
		fprintf(stderr, "number of mapped left arm seqs\t%d\t%.2f\%\n", totlmapped, 100*totlmapped/(totnfq+0.00001));
		fprintf(stderr, "number of uniquely mapped left arm seqs\t%d\t%.2f\%\n", totlunimapped, 100*totlunimapped/(totnfq+0.00001));
		fprintf(stderr, "number of mapped right arm seqs\t%d\t%.2f\%\n", totrmapped, 100*totrmapped/(totnfq+0.00001));
		fprintf(stderr, "number of uniquely mapped right arm seqs\t%d\t%.2f\%\n", totrunimapped, 100*totrunimapped/(totnfq+0.00001));
